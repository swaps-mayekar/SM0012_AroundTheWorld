...
{
    public class CountryCluesGameMode : MonoBehaviour
    {
        ...
        private readonly List<CountryClueQuestion> _questions = new List<CountryClueQuestion>();
        ...
        private void LoadQuestions()
        {
            ...
            {
                Debug.LogError("CountryCluesGameMode: Questions JSON is missing.");
                return;
            }

            var data = JsonUtility.FromJson<CountryClueQuestionList>(questionsJson.text);
            if (data == null || data.questions == null || data.questions.Length == 0)
            {
                Debug.LogError("CountryCluesGameMode: Questions JSON is empty or invalid.");
                return;
            }
            ...
            {
                Debug.LogWarning("CountryCluesGameMode: No questions found for continent " + selectedContinent);
            }
        }
        ...
        private void ShowCurrentQuestion()
        {
            ...
            if (factsText != null)
            {
                factsText.text = BuildFactsText(question);
            }
            else
            {
                Debug.LogWarning("CountryCluesGameMode: Facts text is not assigned.");
            }
        ...
        private string BuildFactsText(CountryClueQuestion question)
        {
            string area = string.IsNullOrWhiteSpace(question.area) ? "N/A" : question.area;
            string population = string.IsNullOrWhiteSpace(question.population) ? "N/A" : question.population;
            string language = string.IsNullOrWhiteSpace(question.language) ? "N/A" : question.language;
            string currency = string.IsNullOrWhiteSpace(question.currency) ? "N/A" : question.currency;
        ...
        private void PrepareAndShowOptions(CountryClueQuestion question)
        {
        ...
        [Serializable]
        private class CountryClueQuestionList
        {
            public CountryClueQuestion[] questions;
        }

        [Serializable]
        private class CountryClueQuestion
        {
